...
class DraggableRect:
    ...
    def move(self, dx, dy):
        self.x += dx; self.y += dy; self.canvas.move(self.tag_root, dx, dy)

    def resize(self, new_w, new_h):
        self.w = max(20, new_w); self.h = max(20, new_h); self.draw() 

    def highlight(self, active=True):
        self.canvas.itemconfig(self.tag_rect, outline="white" if active else COLOR_TEXT_GOLD)
...
class LotroApp:
    ...
    def setup_background(self):
        ...
        if os.path.exists(bg_path):
            try:
                image = Image.open(bg_path); image = image.point(lambda p: p * 0.4) 
    ...
    def resize_background(self, event):
        if hasattr(self, 'bg_image_raw') and event.width > 0:
    ...
    def create_tab(self, parent, title):
        frame = ttk.Frame(parent, style="TFrame"); parent.add(frame, text=title); return frame
    ...
    def setup_status_tab(self):
        ...
        self.lbl_status = ttk.Label(top, text="Warte auf Zeichen...", font=("Georgia", 14, "italic"))
        ...
        self.txt_preview.pack(fill="both", expand=True); self.txt_preview.insert("1.0", "\n   Noch wurde kein Text entrissen...\n"); self.txt_preview.config(state="disabled")
        ...
        def load(p, l):
            if os.path.exists(p):
                try:
                    ...
                    photo = ImageTk.PhotoImage(img); l.config(image=photo, text="", width=0, height=0); return photo
                except: pass
            return None
    ...
    def setup_calibration_tab(self):
        ...
        vs = ttk.Scrollbar(cv, orient="vertical", command=self.calib_canvas.yview)
        hs = ttk.Scrollbar(cv, orient="horizontal", command=self.calib_canvas.xview)
        self.calib_canvas.configure(yscrollcommand=vs.set, xscrollcommand=hs.set)
        vs.pack(side="right", fill="y"); hs.pack(side="bottom", fill="x"); self.calib_canvas.pack(side="left", fill="both", expand=True)
    ...
    def on_mouse_down(self, e):
        if not self.calib_img_raw is None:
            ...
            for i in reversed(items):
                ...
                for name, r in self.template_rects.items():
    ...
    def _do_screenshot(self):
        with mss.mss() as sct:
            ...
            self.calib_photo = ImageTk.PhotoImage(im)
            self.calib_canvas.config(scrollregion=(0,0, im.width, im.height))
    ...
    def setup_settings_tab(self):
        ...
        ttk.Label(sf, text="API Key (Kostenlos von Google AI Studio):").pack(anchor="w")
        self.ent_gemini_key = self.create_entry(sf, show="*"); self.ent_gemini_key.pack(fill="x", pady=5)
        
        self.var_use_ai = tk.BooleanVar()
        tk.Checkbutton(sf, text="Nutze Google Gemini AI statt Tesseract (Besser, braucht Internet)", 
                       variable=self.var_use_ai, bg=COLOR_BG_PANEL, fg=COLOR_TEXT_GOLD, selectcolor=COLOR_BG_DARK).pack(anchor="w", pady=5)
        # -----------------------
        ...
        ttk.Label(sf, text="Monitor ID", style="Header.TLabel").pack(anchor="w", pady=(15,0))
        self.cmb_monitor = ttk.Combobox(sf, values=["1", "2", "3"], state="readonly"); self.cmb_monitor.pack(fill="x", pady=5)

        ttk.Label(sf, text="Hotkey", style="Header.TLabel").pack(anchor="w", pady=(15,0))
        self.ent_hotkey = self.create_entry(sf); self.ent_hotkey.pack(fill="x", pady=5)
        
        self.var_debug = tk.BooleanVar()
        tk.Checkbutton(sf, text="Debug Modus", variable=self.var_debug, bg=COLOR_BG_PANEL, fg=COLOR_TEXT_GOLD, selectcolor=COLOR_BG_DARK).pack(anchor="w", pady=15)
    ...
    def load_settings_to_ui(self):
        ...
        self.ent_gemini_key.insert(0, c.get("gemini_api_key", "")) # NEU
        self.var_use_ai.set(c.get("use_ai_ocr", False))            # NEU
    ...
    def save_settings(self):
        ...
        c["gemini_api_key"] = self.ent_gemini_key.get().strip() # NEU
        c["use_ai_ocr"] = self.var_use_ai.get()                 # NEU
